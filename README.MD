...
> </br>분산락, 카프카+카프카 스트림즈를 활용한 락 관리, 레디스를 활용한 락 관리 등 다양한 락 관리 방법이 있지만, 이번 프로젝트에서는 비관적락을 채택하여 구현함.
> </br> 레디스를 활용한 락 관리는 레디스의 특성상, 메모리에 저장되기 때문에, 레디스 서버가 다운되면 락이 해제되는 문제가 있음.
> </br> 카프카를 활용한 방법의 경우 카프카의 특성상, 메시지의 순서가 보장되지 않기 때문에, 락을 획득한 순서대로 락을 해제할 수 없는 문제가 있음.
> </br> 따라서, 먼저 데이터베이스 락을 활용한 방법을 구현하여야 레디스 및 카프카가 다운되는 상황에서도 락을 유지할 방법을 마련할 수 있을 거라 판단하여 우선 비관적락을 채택함.
> </br> 먼저 비관락으로 구현하고, 성능이슈가 발생하는 상황을 만들어 성능을 개선하는 방향으로 진행할 예정.
