...
2. 사용자 포인트 조회
3. 결제 승인 요청
    1. 결제 금액, 사용자 식별 id, 결제 식별 id로 결제 등록
4. 사용자 포인트 충전

동시성 이슈가 발생하는 부분

포인트 충전이 동시에 두 번이상 발생되어 갱신 손실 발생. 혹은 발생된 트랜잭션만큼 포인트 충전 및 결제승인 요청 발생(일명, 딸깍이)

#### 대응책

낙관적락

- Timestamp 등을 Version 역할을 하는 컬럼으로 지정
- lost update가 발생하는 시점에 대해서는 중복 결제를 막을 수 있지만,

비관적락

- 이 기법 역시 갱신 손실은 막지만, 포인트 중복 결제는 막을 수 없음
- 이 부분은 cs(Customer service)로 해결이 될수도 있음. 그러나, 우리는 개발자니, 기술적인 해결책에 집중하자

**멱등키[채택]**

- 결제시 요청 헤더에 멱등키를 포함하고, 같은 키에 대한 요청이 발생하면, 첫 포인트 충전 결제에 대한 결과만 리턴
- 멱등키는 PG사 연동시, 많이 활용되기도 한다.
- 특별한 동시성 제어를 수행하지 않아도 포인트 충전에 대한 중복 결제 및 갱신 손실을 막을 수 있음
- 결제가 확정되는 지점에서 수집할 수 있는 정보를 조합하여, 멱등키로 활용
    - 사용자 id
    - 토큰 id
    - 가격
    - 충전
