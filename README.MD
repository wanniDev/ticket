...
> - 카프카를 활용한 방법의 경우 카프카의 특성상, 메시지의 순서가 보장되지 않기 때문에, 락을 획득한 순서대로 락을 해제할 수 없는 문제가 있음.
> - 따라서, 먼저 데이터베이스 락을 활용한 방법을 구현하여야 레디스 및 카프카가 다운되는 상황에서도 락을 유지할 방법을 마련할 수 있을 거라 판단하여 우선 비관적락을 채택함.
